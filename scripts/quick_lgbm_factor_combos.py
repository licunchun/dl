"""Quick LightGBM factor-combination comparison for the May 2026 slice."""
...
def _combo_specs(groups: dict[str, list[str]]) -> dict[str, list[str]]:
    ...
    return {
        "value_momentum": cols("value_size", "wq_momentum"),
        "value_momentum_tech": cols("value_size", "wq_momentum", "technical_trend"),
        "value_momentum_pricevol": cols("value_size", "wq_momentum", "wq_price_volume"),
        "value_momentum_moneyflow": cols("value_size", "wq_momentum", "moneyflow"),
        "momentum_pricevol_reversal": cols("wq_momentum", "wq_price_volume", "wq_reversal"),
        "technical_liquidity_moneyflow": cols("technical_trend", "vol_liquidity", "moneyflow"),
        "all_wq": cols("wq_momentum", "wq_reversal", "wq_price_volume"),
        "all_core": cols("value_size", "wq_momentum", "technical_trend", "wq_price_volume"),
        "all_groups": cols(
            "return_rank", "technical_trend", "vol_liquidity", "value_size",
            "moneyflow", "wq_momentum", "wq_reversal", "wq_price_volume",
        ),
    }
...
def main() -> None:
    ...
    for combo_name, raw_cols in combos.items():
        ...
        for direction in ("forward", "inverse"):
            tag = f"combo_{combo_name}_huber_{direction}"
            cand = Candidate(tag, combo_name, "huber", direction, cols)
            print(f"[combo-lgbm] fitting {tag} raw_features={len(raw_cols)} selected={len(cols)}")
            ...
            print(json.dumps({
                ...
                "ic": row["ic"],
                "rank_ic": row["rank_ic"],
                "bt_annualised": row["bt_annualised"],
    ...
    summary_path = out_dir / f"summary_factor_lgbm_combos_{suffix}.csv"
    summary.to_csv(summary_path, index=False)
    (out_dir / f"summary_factor_lgbm_combos_{suffix}.json").write_text(
        json.dumps(rows, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    print(f"[combo-lgbm] wrote {summary_path}")
    print(summary[[
        "tag", "factor_group", "direction", "features", "ic", "rank_ic",
        "top10_spread_bp", "bt_annualised", "bt_sharpe", "bt_max_drawdown", "bt_n_days",
    ]].head(20).to_string(index=False))
    print(f"[combo-lgbm] done in {(time.monotonic() - started) / 60.0:.1f} min")
